...
function parseTopDonations($: CheerioAPI): CafecitoDonation[] {
  ...
  $("header").each((_, el) => {
    ...
    if (!text.includes("top donadores")) return;
    ...
    ul.children("li").each((idx, li) => {
      ...
      const num = Number.parseInt(countText.replace(/[^\d]/g, ""), 10);

      if (name && Number.isFinite(num)) results.push({ name, count: num });
    });
  });
...
function parseLatestCoffees($: CheerioAPI): CafecitoDonation[] {
  ...
  $("section").each((_, sec) => {
    ...
    if (!sectionText.includes("te compró")) return;
    ...
    if (name) {
      ...
      const num = Number.parseInt(lastPart.replace(/[^\d]/g, ""), 10);
      const finalName = name.replace(lastPart, "").trim();
